...
class Allocater implements Callable<List<SimpleFeature>> {
  ...
  private SimpleFeature allocateParcel(SimpleFeatureSource pointPriorityFeatures, SimpleFeature parcelOfInterest,
    ...
    SimpleFeatureType ft = augmentType("Allocated Parcel", parcelOfInterest.getFeatureType(), landUseAttribute,
        String.class);
    try {
      ...
      while (pointFeatures.hasNext()) {
        ...
        final Object attr = comparisonPoint.getAttribute(landUseAttribute);
        if (attr != null) {
          String landUse = attr.toString();
          ...
          if (priorityOrder.containsKey(landUse)) {
            ...
            LOGGER.debug("Missing priority value for classification " + landUse);
          }
        }
      }
  ...
  private SimpleFeatureType augmentType(String name, SimpleFeatureType featureType, String landUseAttribute2,
      Class<?> type) {
